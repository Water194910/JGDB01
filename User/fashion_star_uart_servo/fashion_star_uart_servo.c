...
FSUS_STATUS FSUS_ServoAngleReset(Usart_DataTypeDef *usart, uint8_t servo_id){
	...
	PackageTypeDef pkg;
	return FSUS_RecvPackage(usart, &pkg);
}
...
{
	HAL_Delay(1000);
	FSUS_StopOnControlMode(&FSUS_usart2,0,0,0);//设置0号舵机，以卸力模式，功率为0
	FSUS_StopOnControlMode(&FSUS_usart2,1,0,0);//设置1号舵机，以卸力模式，功率为0
	HAL_Delay(10);
	FSUS_ServoAngleReset(&FSUS_usart2,0);//重置0号舵机圈数
	FSUS_ServoAngleReset(&FSUS_usart2,1);//重置0号舵机圈数
	HAL_Delay(10);
}
